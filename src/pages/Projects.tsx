...
import { updateTempProject, deleteTempProject } from '@/utils/tempProjectUtils';
...
const Projects = () => {
    ...
    try {
      ...
      if (tempProjectsJSON) {
        ...
        const tempProjectsWithVulns = loadedProjects.map(project => {
          ...
          // Check if this is a retest project - improved logic
          const isRetest = project.title.toLowerCase().includes('re-test') || 
                           project.title.toLowerCase().includes('retest') ||
                           project.title.startsWith('Re-test:') ||
                           project.title.startsWith('Retest:');
  ...
  const handleDeleteProject = async (id: string) => {
    try {
      // Handle temporary projects - clean up localStorage before API call
      if (id.startsWith('temp_')) {
        // Use the utility function to delete temporary project
        const success = deleteTempProject(id);
        ...
        if (success) {
          ...
          toast({
            title: 'Success',
            description: 'Temporary project deleted successfully',
          });
        } else {
      ...
      // Make API call (works for both temp and regular projects)
      ...
      // Update the UI based on project type (for regular projects)
      if (!id.startsWith('temp_')) {
  ...
  const markProjectAsCompleted = async (projectId: string) => {
    try {
      // Handle temporary projects with localStorage
      if (projectId.startsWith('temp_')) {
        console.log('Marking temporary project as completed:', projectId);
        const success = updateTempProject(projectId, { status: 'completed' });
        ...
        if (success) {
          ...
          toast({
            title: 'Success',
            description: 'Temporary project marked as completed',
          });
        } else {
          throw new Error('Failed to update temporary project status');
        }
        return;
      }
      
      // Handle cloud projects with API
      ...
      // Update the local state for cloud projects
  ...
  // Enhanced filtering functions for each tab - Fixed retest filtering
  const getFilteredProjectsForTab = (tab: string) => {
    switch (tab) {
      case 'all':
        return sortedProjects;
      case 'temp':
        return sortedProjects.filter(p => p.isTemporary);
      case 'draft':
        return sortedProjects.filter(p => p.status === 'draft');
      case 'completed':
        return sortedProjects.filter(p => p.status === 'completed');
      case 'retest':
        // Enhanced retest filtering - check multiple patterns and is_retest flag
        return sortedProjects.filter(p => {
          const titleCheck = p.title.toLowerCase().includes('re-test') || 
                           p.title.toLowerCase().includes('retest') ||
                           p.title.startsWith('Re-test:') ||
                           p.title.startsWith('Retest:');
          
          return p.is_retest || titleCheck;
        });
      case 'initial':
        // Initial assessment - exclude retest projects
        return sortedProjects.filter(p => {
          const titleCheck = p.title.toLowerCase().includes('re-test') || 
                           p.title.toLowerCase().includes('retest') ||
                           p.title.startsWith('Re-test:') ||
          ...
          return !p.is_retest && !titleCheck && !p.isTemporary;
        });
      default:
        return sortedProjects;
    }
  ...
  const renderProjects = (projectsToRender: ProjectWithVulnerabilities[]) => {
    if (loading) {
      return (
        ...
        <Table>
          ...
          <TableBody>
            {projectsToRender.map((project) => {
              // Determine if this is a retest project by checking multiple patterns
              const isRetestProject = project.is_retest || 
              ...
              return (
                <TableRow key={project.id}>
                  ...
                  <TableCell className="text-right">
                    {/* ... keep existing code (dropdown menu) the same ... */}
                    <DropdownMenu>
    ...
    <MainLayout>
      ...
      <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="all" className="space-y-4">
          {renderProjects(getFilteredProjectsForTab('all'))}
        </TabsContent>

        <TabsContent value="initial" className="space-y-4">
          {renderProjects(getFilteredProjectsForTab('initial'))}
        </TabsContent>

        <TabsContent value="temp" className="space-y-4">
          {renderProjects(getFilteredProjectsForTab('temp'))}
        </TabsContent>

        <TabsContent value="draft" className="space-y-4">
          {renderProjects(getFilteredProjectsForTab('draft'))}
        </TabsContent>

        <TabsContent value="review" className="space-y-4">
          {renderProjects(getFilteredProjectsForTab('review'))}
        </TabsContent>

        <TabsContent value="completed" className="space-y-4">
          {renderProjects(getFilteredProjectsForTab('completed'))}
        </TabsContent>

        <TabsContent value="retest" className="space-y-4">
          {renderProjects(getFilteredProjectsForTab('retest'))}
        </TabsContent>
      </Tabs>
