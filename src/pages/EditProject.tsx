...
import { isTemporaryProject, getTempProject, updateTempProject } from '@/utils/tempProjectUtils';
...
const EditProject = () => {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        const isTemporary = isTemporaryProject(projectId);
        setIsTemp(isTemporary);

        let data;
        if (isTemporary) {
          // Get from localStorage
          data = getTempProject(projectId);
          if (!data) {
        ...
        if (data) {
          ...
          setValue('isRetest', data.title.startsWith('Re-test:') || data.title.startsWith('Retest:'));
        }
      } catch (error: any) {
        toast({
          title: 'Error',
  ...
  const onSubmit = async (data: ProjectFormData) => {
    try {
      ...
      const isCurrentlyRetest = title.startsWith('Re-test:') || title.startsWith('Retest:');
      
      if (data.isRetest && !isCurrentlyRetest) {
        updatedTitle = `Re-test: ${title.replace(/^(Re-test:|Retest:)\s*/, '')}`;
      } else if (!data.isRetest && isCurrentlyRetest) {
        updatedTitle = title.replace(/^(Re-test:|Retest:)\s*/, '');
      }
      ...
      let result;
      if (isTemp) {
        // Update temporary project in localStorage
        const success = updateTempProject(projectId!, updateData);
        result = { success };
      } else {
        // Update regular project via API
        result = await reportsApi.update(projectId!, updateData);
      }
