...
import { ChevronDown, ChevronLeft, FileText, PenLine, Plus, Shield, Trash2, Archive, BarChart3, Hammer, CheckCircle, RotateCcw } from 'lucide-react';
...
const ProjectDetails = () => {
          ...
          if (tempProjectsJSON) {
            ...
            if (tempProject) {
              ...
              const projectWithDefaults = {
                ...tempProject,
                status: tempProject.status || 'draft'
              };
  ...
  const handleUpdateProjectStatus = async (newStatus: string) => {
    ...
    try {
      if (isTemporary) {
        ...
        // For cloud projects, update via API
        const result = await reportsApi.update(projectId!, { status: newStatus });
        if (!result.success) {
  ...
  // Ensure status has a fallback value for rendering
    ...
    <MainLayout>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <Badge variant={projectStatus === 'completed' ? 'default' : 'outline'} className="ml-2">
        ...
        <div className="flex gap-2">
          ...
          {/* Conditional Status Action Button */}
          <DropdownMenu>
